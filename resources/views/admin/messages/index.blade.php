        ...
        <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
            <div class="shadow ring-1 ring-black dark:ring-gray-700 ring-opacity-5 dark:ring-opacity-100 sm:rounded-lg">
                @forelse($messages as $message)
                    <div class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 {{ $message->read ? '' : 'bg-indigo-50 dark:bg-indigo-900/20' }}">
                        <div class="px-4 py-5 sm:px-6">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-3">
                                    ...
                                    @if($message->status === 'pending')
                                        <span class="inline-flex items-center rounded-md bg-yellow-100 dark:bg-yellow-900/30 px-2.5 py-0.5 text-xs font-medium text-yellow-800 dark:text-yellow-400">
                                            Väntar
                                        </span>
                                    ...
                                    @if($message->priceEstimation)
                                        <span class="inline-flex items-center rounded-md bg-purple-100 dark:bg-purple-900/30 px-2.5 py-0.5 text-xs font-medium text-purple-800 dark:text-purple-400" title="Har kopplad prisestimering">
                                            📊 Estimering
                                        </span>
                                    @endif
                                </div>
                            ...
                            <div class="mt-4">
                                <p class="text-sm text-gray-700 dark:text-gray-300 line-clamp-2">{{ $message->message }}</p>
                            </div>
